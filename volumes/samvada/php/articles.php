...
if($letter == 'Special')
{
	$query = "select * from article where title not regexp '^[a-zA-Z].*' order by title, volume, page_start";
}
else
{
	$query = "select * from article where title like '$letter%' order by title, volume, page_start";
}

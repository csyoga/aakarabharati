...
if($letter == 'Special')
{
	$query = "select * from article_samvada where title not regexp '^[a-zA-Z].*' order by title, volume, page_start";
}
else
{
	$query = "select * from article_samvada where title like '$letter%' order by title, volume, page_start";
}

...
if($letter == 'Special')
{
	$query = "select * from article_maatukate where title not regexp '^[a-zA-Z].*' order by title, part, page_start";
}
else
{
	$query = "select * from article_maatukate where title like '$letter%' order by title, part, page_start";
}

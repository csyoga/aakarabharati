...
($isVolumePart === 'true') ? $query = 'SELECT * FROM journals WHERE journalid = \'' . $journalID . '\'  AND feature regexp \'' . addslashes($feature) . '\' order by  titleid' : $query = 'SELECT * FROM article WHERE journalid = \'' . $journalID . '\' AND feature regexp \'' . $feature . '\' order by  titleid' ;
$result = $db->query($query); 
$num_rows = $result ? $result->num_rows : 0;

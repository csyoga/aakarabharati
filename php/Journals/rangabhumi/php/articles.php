...
$query = 'select * from article_rb where title like \'' . $letter . '%\' order by volume, part, title, page_start';
...
if($num_rows > 0)
{
	...
	{
		$query3 = 'select feat_name from feature_rb where featid=\'' . $row['featid'] . '\'';
...
{
	echo '<span class="sml">Sorry! No articles were found to begin with the letter \'' . $letter . '\' in Rangabhumi</span>';
}

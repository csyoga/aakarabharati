...
$query = 'select * from article_sakshi where featid=\'' . $featid . '\' order by  part, page_start';
...
if($num_rows > 0)
{
	while($row = $result->fetch_assoc())
	{
		...
		echo '	</div>';
		echo '	<span class="aTitle"><a target="_blank" href="../../../../Volumes/sakshi/' . $row['part'] . '/index.djvu?djvuopts&amp;page=' . $row['page_start'] . '.djvu&amp;zoom=page">' . $row['title'] . '</a></span><br />';
		if($row['authid'] != 0) {

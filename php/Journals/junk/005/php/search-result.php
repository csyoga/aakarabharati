...
if($text=='')
{
    $query="SELECT * FROM
                (SELECT * FROM
                    (SELECT * FROM
                        (SELECT * FROM article_ab WHERE $authorFilter) AS tb1
                    WHERE $titleFilter) AS tb2
                WHERE featid REGEXP '$featid') AS tb3
            ORDER BY  titleid, part, page_start";
...
if($num_rows > 0)
{
    ...
    {
        $query3 = 'select feat_name from feature_ab where featid=\'' . $row['featid'] . '\'';
       ...
       echo ($row3['feat_name'] != '') ? '		<span class="aFeature clr2"><a href="feat.php?feature='.$row3['feat_name'].'&amp;featid='.$row['featid'].' ">' . $row3['feat_name'] . '</a></span> | ' : '';
		...
		echo '	</div>';
		echo '	<span class="aTitle"><a target="_blank" href="../../../../Volumes/'. 'arivubaraha'.'/'. $row['part'] . '/index.djvu?djvuopts&amp;page=' . $row['page_start'] . '.djvu&amp;zoom=page_start">' . $row['title'] . '</a></span><br />';
		if($row['authid'] != 0) {

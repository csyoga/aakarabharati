...
if($text=='')
{
    $query="SELECT * FROM
                (SELECT * FROM
                    (SELECT * FROM
                        (SELECT * FROM article_pp WHERE $authorFilter) AS tb1
...
if($num_rows > 0)
{
    while($row = $result->fetch_assoc())
    {
       ...
       echo ($row3['feat_name'] != '') ? '		<span class="aFeature clr2"><a href="feat.php?feature='.$row3['feat_name'].'&amp;featid='.$row['featid'].' ">' . $row3['feat_name'] . '</a></span> | ' : '';
		...
		echo '	</div>';
		echo '	<span class="aTitle"><a target="_blank" href="../../../../../Volumes/'. 'pustakaprapancha'.'/'. $row['volume'] .'/'. $row['part'] . '/index.djvu?djvuopts&amp;page=' . $row['page_start'] . '.djvu&amp;zoom=page_start">' . $row['title'] . '</a></span><br />';
		if($row['authid'] != 0) {

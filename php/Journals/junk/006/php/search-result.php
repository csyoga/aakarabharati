...
if($text=='')
{
    $query="SELECT * FROM
                (SELECT * FROM
                    (SELECT * FROM
                        (SELECT * FROM article_rb WHERE $authorFilter) AS tb1
...
if($num_rows > 0)
{
    while($row = $result->fetch_assoc())
    {
       ...
       echo ($row3['feat_name'] != '') ? '		<span class="aFeature clr2"><a href="feat.php?feature='.$row3['feat_name'].'&amp;featid='.$row['featid'].' ">' . $row3['feat_name'] . '</a></span> | ' : '';
		echo '		<span class="aIssue clr5"><a href="toc.php?volume='.$row['volume'].'\'&amp;part='. $row['part'] .'">ಸಂಪುಟ '.intval($row['volume']).'&nbsp;;&nbsp;ಸಂಚಿಕೆ ' . intval($dpart) . '</a></span>';		echo '	</div>';

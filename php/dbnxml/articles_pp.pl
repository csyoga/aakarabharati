...
$sth11r=$dbh->prepare("CREATE TABLE article_pp(title varchar(1500), 
authid varchar(20),
...
while($line)
{
	...
	{
		insert_article($title,$authids,$author_name,$featid,$year,$month,$type_code,$page_start,$page_end,$volume,$pnum,$info);
		$authids = "";
		$featid = "";
		$author_name = "";
		$id = "";
...
sub insert_article()
{
	my($title,$authids,$author_name,$featid,$year,$month,$type_code,$page_start,$page_end,$volume,$pnum,$info) = @_;
	my($sth1);
	...
	$author_name =~ s/^;//;
	$author_name =~ s/'/\\'/g;
	$sth1=$dbh->prepare("insert into article_pp values('$title','$authids','$author_name','$featid','$year','$month','$type_code','$page_start','$page_end','$volume','$pnum','$info','')");
...
sub get_featid()
{
	...
	$sth=$dbh->prepare("select featid from feature_pp where feat_name='$feature'");
	$sth->execute();

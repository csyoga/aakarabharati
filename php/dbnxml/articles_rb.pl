...
$sth11r=$dbh->prepare("CREATE TABLE article_rb(title varchar(1500), 
authid varchar(20),
...
sub insert_article()
{
	...
	$author_name =~ s/^;//;
	$author_name =~ s/'/\\'/g;
	$sth1=$dbh->prepare("insert into article_rb values('$title','$authids','$author_name','$featid','$year','$month','$play','$type_code','$page_start','$page_end','$volume','$pnum','$info','')");
...
sub get_featid()
{
	...
	$sth=$dbh->prepare("select featid from feature_rb where feat_name='$feature'");
	$sth->execute();

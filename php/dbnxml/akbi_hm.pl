...
open(IN,"hm.xml") or die "can't open hm.xml\n";

my $dbh=DBI->connect("DBI:mysql:database=$db;host=$host","$usr","$pwd");
$dbh->{'mysql_enable_utf8'} = 1;
$dbh->do('SET NAMES utf8');

$sth1=$dbh->prepare("CREATE TABLE akbi_hm(
book_id varchar(4), 
level int(2),
title varchar(1000),
authid varchar(200),
...
sub insert_to_db()
{
	...
	$sth2=$dbh->prepare("insert into akbi_hm values('$book_id','$level','$title','$authid','$authors','$page','$page_end','$type','')");
	$sth2->execute();
	$sth2->finish();
}

<?php include("header.php");	?>
<?php include("nav.php"); ?>
	<main class="cd-main-content">
		<section id="about">
			<h2>ಪುಟ ಸೂಚಕ</h2>
			<div id="about_p">
		<ul class="about_ul">
			<li><a href="../index.html">ಮುಖಪುಟ</a></li>
			<ul>
				<li><a href="index.php">ಆಕರಭಾರತಿ</a></li>
				<li><a href="about.php">ಪಕ್ಷಿನೋಟ</a></li>
				<li><a href="credits.php">ಕೃತಜ್ಞತೆಗಳು</a></li>
				<li><a href="#">ಗ್ಯಾಲರಿ</a></li>
				<li><a href="contact.php">ಸಂಪರ್ಕ</a></li>
				<li><a href="#">ಹುಡುಕು</a></li>
			</ul>
			<ul>
				<li><a href="index.php">ಸಂಗ್ರಹ</a></li>
				<li><a href="">ನಿಯತಕಾಲಿಕೆ</a>
				    <ul>
				       <li><a href="Journals/sakshi/index.php">ಸಾಕ್ಷಿ</a></li>
				       <li><a href="Journals/samvada/index.php">ಸಂವಾದ</a></li>
				       <li><a href="Journals/maatukate/index.php">ಮಾತುಕತೆ</a></li>
				       <li><a href="Journals/pustakaprapancha/index.php">ಪುಸ್ತಕ ಪ್ರಪಂಚ</a></li>
				       <li><a href="Journals/arivubaraha/index.php">ಅರಿವು ಬರಹ</a></li>
				       <li><a href="Journals/rangabhumi/index.php">ರಂಗಭೂಮಿ</a></li>
				       <li><a href="#">ಗ್ರಂಥಲೋಕ</a></li>
				       <li><a href="#">ಪ್ರಬುದ್ಧ ಕರ್ಣಾಟಕ</a></li>
				    </ul>
				</li>
				<li><a href="Journals/index.html">ಸಮಗ್ರ ಸಾಹಿತ್ಯ</a>
					<ul>
						<li><a href="#">ಮಂಜೇಶ್ವರ ಗೋವಿಂದ ಪೈ</a></li>
						<li><a href="#">ಫು. ಗು. ಹಳಕಟ್ಟಿ</a></li>
						<li><a href="#">ಹರ್ಡೇಕರ್ ಮಂಜಪ್ಪ</a></li>
					</ul>
				</li>
				<li><a href="article_list/index.html">ಲೇಖನ</a>
					<ul>
						<li><a href="blog/kgv.html">ಆಲೂರು ವೆಂಕಟರಾವ್ </a></li>
					</ul>
				</li>
				<li><a href="article_list/index.html">ಬಿಡಿ ಬರಹ</a>
					<ul>
						<li><a href="#">ಕೆ. ಎಸ್. ಮಧುಸೂದನ್</a></li>
						<li><a href="#">ಡಾ. ಅಶ್ವಿನ್ ಕುಮಾರ್ ಎ. ಪಿ</a></li>
					</ul>
				</li>
				<li><a href="article_list/index.html">ಸಂಶೋಧನಾ ಪ್ರಬಂಧ</a>
					<ul>
						<li><a href="blog/niranjanavanalli.html">ನಿರಂಜನ ವಾನಳ್ಳಿ</a></li>
						<li><a href="#0">ನಂಜುಂಡಸ್ವಾಮಿ</a></li>
					</ul>
				</li>
				<li><a href="Journals/index.html">ಪುಸ್ತಕ</a>
					<ul>
						<li><a href="">ಕೃಷ್ಣ ಮಹೀಶ್ವರೀ</a></li>
						<li><a href="">ಸುವ್ರತ</a></li>
						<li><a href="">ಮಕ್ಕಳ ಕಲಿಕೆಯ ವಸ್ತುಪಾಠಗಳು</a></li>
						<li><a href="">ತಮಿಳ ಮೊದಲ ಪುಸ್ತಕ</a></li>
						<li><a href="">ಭಾಷಾಮಂಜರಿ</a></li>
						<li><a href="">ಗಣಿತಾರ್ಣವ</a></li>
						<li><a href="Journals/index.html">ಪ್ರಶ್ನೋತ್ತರ ಲೇಖ್ಯಬೋಧಿನಿ</a></li>
					</ul>
				</li>
			</ul>
			<ul>
				<li><a href="">ಆಯಾಮ</a></li>
			</ul>
			<ul>
				<li><a href="">ಭಾಗವಹಿಸಿ</a></li>
			</ul>
			<li><a href="sitemap.php">ಪುಟ ಸೂಚಕ</a></li>
		</ul>
			</div>
	  </section>
	</main>
	<div id="cd-search" class="cd-search">
		<form>
			<input type="search" placeholder="Comming Soon...">
		</form>
	</div>
<?php include("footer.php"); ?>
	


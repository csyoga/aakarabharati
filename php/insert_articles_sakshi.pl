...
while($line)
{
	...
	{
		insert_article($title,$authids,$author_name,$featid,$rutu,$type,$page_start,$page_end,$volume,);
		$authids = "";
		$featid = "";
		$author_name = "";
		$id = "";
...
sub insert_article()
{
	my($title,$authids,$author_name,$featid,$rutu,$type,$page_start,$page_end,$volume) = @_;
	my($sth1);
	...
	$author_name =~ s/^;//;
	$author_name =~ s/'/\\'/g;
	$sth1=$dbh->prepare("insert into article_sakshi values('$title','$authids','$author_name','$featid','$rutu','$type','$page_start','$page_end','$volume','')");
